# -*- coding: utf-8 -*-
# BUILD_CODE share codes/export text have changed several times in the same
# sitting: compression moved from gzip to raw DEFLATE ("deflate-raw" - same
# underlying compression, but without gzip's 10-byte header + 8-byte trailer
# a code embedded in a URL/text blob has no use for); the JSON payload moved
# from a keyed object ({v,c,l,r,p,o,w}, v2) to a positional array
# ([v,c,l,r,p,o,w], v3+); and r/o's own inner shape moved from an array of
# [id,rank] pairs to columnar ([[ids...],[ranks...]], v4+) - separating
# same-typed values into their own runs compresses noticeably better than
# an interleaved id/rank/id/rank/... sequence does (see exportImport.js's
# compress/decompress, buildCodeArray, expandCompactPayload,
# expandCompactRanks). decodeBuildCode checks gzip's own magic bytes
# directly, then falls back to deflate-raw and finally plain uncompressed
# JSON if that fails too - format-sniffed on the bytes themselves rather
# than gated on BUILD_CODE_VERSION, so every era of link/export keeps
# working forever, independent of which JSON shape ends up inside once
# decompressed.
#
...
# plain-JSON code, a v2 keyed-object code, and a v3 positional-but-pair-
# shaped code for the SAME current build would have looked like, directly
# from the browser's own CompressionStream - then feeds each into Import
# and confirms they all load identically to the app's own current export.
# Also confirms the real, measured savings: the app's own current code is
# shorter than the gzip equivalent for the same content.
import os, sys, io
...
with sync_playwright() as p:
    ...
    async (currentCode) => {
      ...
      // The current code's r/o are columnar ([[ids...],[ranks...]],
      // BUILD_CODE_VERSION 4+) - converting back to the older array-of-
      // pairs shape ([[id,rank],[id,rank],...]) a v2/v3 code actually used.
      function toPairs(columnar) {
        return columnar ? columnar[0].map((id, i) => [id, columnar[1][i]]) : columnar;
      }

      const arr = JSON.parse(new TextDecoder().decode(jsonBytes));
      const rPairs = toPairs(arr[3]);
      const oPairs = toPairs(arr[5]);

      // A genuine v2 keyed-object payload for the exact same content - not
      // just re-wrapping the current array, actually reshaping it into the
      // old container AND the old pair-shaped r/o, so both
      // expandCompactPayload's Array.isArray branch and
      // expandCompactRanks' columnar branch are deliberately exercised,
      // not just incidentally covered by some other test's aging fixture.
      const v2Payload = { v: 2, c: arr[1], l: arr[2], r: rPairs, p: arr[4] };
      if (oPairs) v2Payload.o = oPairs;
      ...
      // A genuine v3 payload: the current positional-array container, but
      // r/o still pair-shaped (BUILD_CODE_VERSION 3, before columnar) -
      // pins the array-container-but-pair-shaped combination specifically,
      // independent of the v2 (keyed-object) case above.
      const v3Array = [3, arr[1], arr[2], rPairs, arr[4], oPairs, arr[6]];
      const v3Bytes = new TextEncoder().encode(JSON.stringify(v3Array));
      const v3GzipBytes = await pipe(v3Bytes, CompressionStream, "gzip");

      return {
        gzipCode: bytesToB64(gzipBytes),
        plainCode: bytesToB64(jsonBytes),
        v2Code: bytesToB64(v2GzipBytes),
        v3Code: bytesToB64(v3GzipBytes),
    ...
    # --- A genuine v3 code (positional array, but pair-shaped r/o rather
    # than the current columnar shape) also still imports identically -
    # pins the array-container-but-pair-shaped combination specifically. ---
    page.click("#importBtn")
    page.wait_for_timeout(100)
    page.fill("#importText", reconstructed["v3Code"])
    page.click("#doImportBtn")
    page.wait_for_timeout(200)
    spent_after_v3_import = page.locator("#spentValue").inner_text()
    print("spentValue after importing a v3 pair-shaped code:", spent_after_v3_import)
    assert spent_after_v3_import == spent_before, "FAIL: a v3 pair-shaped code should still import correctly"
    print("PASS: a v3 pair-shaped code still imports correctly")
